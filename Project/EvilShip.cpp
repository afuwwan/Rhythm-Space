...
/*
Engine::EvilShip::EvilShip(Texture* texture, Shader* shader, Quad* quad) :BaseCharacter(texture, shader, quad)
{
...
Engine::EvilShip::~EvilShip()
{
}

void Engine::EvilShip::Update(float deltaTime)
{
...
void Engine::EvilShip::SetTarget(BaseCharacter* target)
{
	this->target = target;
}

void Engine::EvilShip::SetMaxSpeed(float maxSpeed)
{
	this->maxSpeed = maxSpeed;
}

void Engine::EvilShip::Move(float deltaTime)
{
	vec2 steering = Seek();
...
string Engine::EvilShip::FacingSprite(vec2 position, vec2 target)
{
	if (target == position) {
		return "";
	}
